...
async function init() {
  ...
  document.getElementById("enContext").checked = settings.enContext !== false;
  ...
  ["enSel", "enInput", "enFloat", "enContext"].forEach((id) => {
    document.getElementById(id).addEventListener("change", save);
  });
  ...
  document.getElementById("blacklistAdd").addEventListener("click", () => {
    ...
    if (!host) return;
    chrome.runtime.sendMessage({ action: "addBlacklist", host }, (r) => {
      if (r && r.success) {
        settings.blacklist = r.blacklist;
        renderBlacklist(r.blacklist);
        input.value = "";
      }
    });
  });
